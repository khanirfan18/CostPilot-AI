...
# Verify if we already have AnalyzerLoading
if "function AnalyzerLoading" not in content:
    loader_code = """
...
content = content.replace(
    ...
    '{isAnalyzing && <AnalyzerLoading />}\n        {!isAnalyzing && result && <AnalysisResults result={result} addToast={addToast} currencySymbol={currency} />}'
)

# And to make "Manual Entry" show it for a bit so the animation is visible, let's add a small artificial delay if activeTab === "manual"
manual_delay = """      if (activeTab === "manual") {
        // Manual calculation
        await new Promise(resolve => setTimeout(resolve, 3000))
        analysisResult = calculateManual()"""
content = re.sub(
    r'      if \(activeTab === "manual"\) \{\n\s*// Manual calculation\n\s*analysisResult = calculateManual\(\)',
    manual_delay,
    content
)
